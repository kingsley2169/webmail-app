<?php
  ...
  if (!isset($_SESSION['unique_id'])) {
    ...
    $sql2 = mysqli_query($conn, "SELECT * FROM messages WHERE outgoingMsgId = '$id' ORDER BY messageId DESC");
    if(mysqli_num_rows($sql2)>0) {
      while ($row1 = mysqli_fetch_assoc($sql2)) {
        if ($row1['outgoingMsgId'] === $id) {

          $inboxField .= '<tr class="messageListItem">
                          ...
                          <td class="mailFrom">'. $row1['incomingMsgId'].'</td>
